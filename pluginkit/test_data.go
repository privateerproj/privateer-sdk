...
var activeTestCatalog = &gemara.Catalog{
	Controls: []gemara.Control{},
}

func getTestCatalog(t *testing.T) *gemara.Catalog {
	if len(activeTestCatalog.Controls) > 0 {
		return activeTestCatalog
	}
	pwd, err := os.Getwd()
	if err != nil {
		if t == nil {
		...
		t.Fatal("could not get working directory when retrieving catalog: %w", err)
	}
	file1 := fmt.Sprintf("file://%s/catalog-test-data/metadata.yaml", pwd)
	file2 := fmt.Sprintf("file://%s/catalog-test-data/controls.yaml", pwd)
	err = activeTestCatalog.LoadFiles([]string{file1, file2})
	if err != nil {
		if t == nil {
			return nil
		}
		t.Fatal("Failed to load test data: %w", err)
	}
	return activeTestCatalog
}

// getEmptyTestCatalog returns an empty catalog for testing error conditions
func getEmptyTestCatalog() *gemara.Catalog {
...
func getTestCatalogWithNoRequirements() *gemara.Catalog {
	return &gemara.Catalog{
		Controls: []gemara.Control{
			{
				Id:                     "test-control",
...
func step_Pass(data interface{}) (result gemara.Result, message string, confidenceLevel gemara.ConfidenceLevel) {
	return gemara.Passed, "This step always passes", 1
}

func step_Fail(_ interface{}) (result gemara.Result, message string, confidenceLevel gemara.ConfidenceLevel) {
	return gemara.Failed, "This step always fails", 1
}

func step_NeedsReview(_ interface{}) (result gemara.Result, message string, confidenceLevel gemara.ConfidenceLevel) {
	return gemara.NeedsReview, "This step always needs review", 1
}
...
func getAllRequirementIds() []string {
	catalog := getTestCatalog(nil)
	if catalog == nil {
		return []string{}
	}
...
func convertEvalsToStepsMap(evals []*gemara.ControlEvaluation) map[string][]gemara.AssessmentStep {
	...
	requirementIds := getAllRequirementIds()
