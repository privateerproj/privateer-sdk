...
func getPluginCatalogs(dataDir string, files fs.FS) (catalogs []*gemara.ControlCatalog, err error) {
	dir, err := fs.ReadDir(files, dataDir)
...
func readYAMLFile(filePath string, files fs.FS) (*gemara.ControlCatalog, error) {
...
// ParseCatalog decodes one Gemara control catalog from YAML. It is the single
// decoder for catalogs the SDK reads, whether embedded in a plugin, read from
// the install cache, or pulled from grc.store at publish time.
func ParseCatalog(data []byte) (*gemara.ControlCatalog, error) {
